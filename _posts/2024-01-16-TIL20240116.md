...
-  Spring Security 아키텍처
  ...
  - 이런 필터에서 인증을 담당하는 것을 구현하는 것이 Spring Security의 핵심
    - 기본 Security 외의 다른 커스텀 필터를 추가하려면 Spring이 제공하는 DelegatingFilterProxy를 사용
    - servlet filter chain과 application context 간의 bridge 역할을 하는 것이 DelegatingFilterProxy이다.
    - Spring Security는 이 DelegatingFilterProxy를 기반으로 동작함
    - 여기서 FilterChainProxy라는 것을 넣음
...
- Spring Security의 인증 처리 과정
  ...
  - 정리하자면 - *인증 필터를 통해 요청을 받아, 인증 여부를 판단하고, 인증이 됐을 경우 SecurityContextHolder에 Authentication 객체를 할당하는 것이 핵심
